...
import { Lock, Wallet, Sparkles, ShieldCheck, Flame, Heart, Play } from "lucide-react";

interface ArcadeLockscreenProps {
  onPlayAsGuest?: () => void;
}

export const ArcadeLockscreen: React.FC<ArcadeLockscreenProps> = ({ onPlayAsGuest }) => {
    ...
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center p-6 bg-slate-950/85 backdrop-blur-md text-center animate-fade-in select-none">
      ...
      <div className="relative z-10 max-w-sm space-y-4">
        ...
        <div className="space-y-1">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-mono font-bold bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 mb-1">
            <Sparkles className="w-3 h-3 text-emerald-400" />
            <span>INSTANT PLAY READY</span>
          </div>
          <h3 className="text-2xl font-black text-white tracking-tight">
            Feed Nomster
          </h3>
          <p className="text-xs text-slate-300 leading-relaxed">
            Play directly in your browser with 5 lives. No wallet or sign up required!
          </p>
        </div>
        ...
        <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800 text-left space-y-2 text-[11px] text-slate-300 font-mono">
          ...
          <div className="flex items-center gap-2">
            ...
            <span><strong>High Scores:</strong> Save to Global Leaderboard &amp; NomWall!</span>
          </div>
        </div>

        {/* Action Buttons: Guest Mode (Primary) & Wallet/Login (Secondary) */}
        <div className="space-y-2 pt-1">
          <button
            onClick={onPlayAsGuest}
            className="w-full py-3.5 px-6 rounded-xl font-black text-sm bg-gradient-to-r from-emerald-400 via-teal-300 to-solana-green text-slate-950 shadow-[0_0_25px_rgba(20,241,149,0.4)] hover:shadow-[0_0_35px_rgba(20,241,149,0.7)] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl font-semibold text-xs bg-slate-900/90 hover:bg-slate-800 text-slate-300 border border-slate-700/80 hover:border-slate-600 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            <Wallet className="w-3.5 h-3.5 text-emerald-400" />
            <span>Connect Wallet / Sign In to Save Score</span>
          </button>
        </div>
      </div>
    </div>
